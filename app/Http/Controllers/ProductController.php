...
class ProductController extends Controller
{
    ...
    public function edit_product1(Request $request, $product_id){
        $this->AuthLogin();

        $currentProduct = DB::table('tbl_product')->where('product_id', $product_id)->first();

        // Tiến hành kiểm tra liệu có thay đổi dữ liệu hay không
        $isDataChanged = false;
        if ($currentProduct->product_quantity != $request->product_quantity ) {
            $isDataChanged = true;
        }
    
        // Nếu có sự thay đổi, cập nhật thời gian cập nhật
        if ($isDataChanged) {
            DB::table('tbl_product')->where('product_id', $product_id)->update([
                'updated_at' => now() // hoặc sử dụng định dạng thời gian phù hợp với bạn
            ]);
        }
       ...
       $manager_product  = view('admin.product.edit_product1')->with('edit_product',$edit_product)->with('cate_product',$cate_product)->with('brand_product',$brand_product);

       return view('admin_layout')->with('admin.product.edit_product1', $manager_product);
   }
    ...
    public function update_product1(Request $request,$product_id){
        $this->AuthLogin();
        $product_price = filter_var($request->product_price,FILTER_SANITIZE_NUMBER_INT);
       $price_cost = filter_var($request->price_cost,FILTER_SANITIZE_NUMBER_INT);
       ...
       // Session::put('message','Cập nhật sản phẩm thành công');
       Toastr::success('Cập nhật sản phẩm thành công','Thành công');
       return Redirect::to('view-export');
   }
    ...
    public function delete_product1($product_id){
        $this->AuthLogin();
        DB::table('tbl_product')->where('product_id',$product_id)->delete();
        Toastr::success('Xóa sản phẩm thành công','Thành công');
        return Redirect::to('view-export');
    ...
    public function view_export(){
            ...
            ->join('tbl_brand','tbl_brand.brand_id','=','tbl_product.brand_id')
            ->orderby('tbl_product.product_id','desc')->get();
            $manager_product  = view('admin.product.view_export')->with('all_product',$all_product);
